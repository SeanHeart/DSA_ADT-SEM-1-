...
void displayLink(StudLink S) {
    ...
    for (;S != NULL; S = S->link) {
        // Concatenate first name and last name into a temporary buffer
        char fullName[40]; // Adjust the size according to your needs
        snprintf(fullName, sizeof(fullName), "%s %s", S->stud.name.FN, S->stud.name.LN);
        printf("%-10s| %-20s| %-3c| %-8s| %-4d\n", S->stud.ID, fullName, S->stud.name.MI, S->stud.course, S->stud.yrLevel);
    }
    printf("--------------------------------------------------------\n");
}
void displayArr(StudArray S) {
    ...
    for (int i = 0; i < S.count; i++) {
        // Concatenate first name and last name into a temporary buffer
        char fullName[40]; // Adjust the size according to your needs
        snprintf(fullName, sizeof(fullName), "%s %s", S.stud[i].name.FN, S.stud[i].name.LN);
        printf("%-10s| %-20s| %-3c| %-8s| %-4d\n", S.stud[i].ID, fullName, S.stud[i].name.MI, S.stud[i].course, S.stud[i].yrLevel);
    }
    printf("--------------------------------------------------------\n");
}

ComboList createComboList(StudLink *S) {
    ComboList CL = malloc(sizeof(struct List));
    if (CL != NULL) {
        CL->CS.count = 0;
        CL->IT = NULL;
        
        StudLink *trav, temp, lastnode_CS = NULL, lastnode_IT = NULL;
        for (trav = S; *trav != NULL;) {
            if (strcmp((*trav)->stud.course, "BSCS") == 0) {
                temp = *trav;
                if (CL->CS.count < MAX) {
                    CL->CS.stud[CL->CS.count++] = temp->stud;
                }
                *trav = temp->link;
                free(temp);
                ...
                *trav = temp->link;
                ...
                if (CL->IT == NULL) {
                    ...
                    lastnode_IT->link = temp;
                }
                lastnode_IT = temp;
            } else {
                trav = &(*trav)->link;
            }
            if (*trav != NULL && strcmp((*trav)->stud.course, "BSCS") != 0) {
                lastnode_CS = *trav;
            }
        }
        if (lastnode_CS != NULL) {
            lastnode_CS->link = NULL;
        }
    }
    return CL;
}
